...
updt_onra <- function(update, X, Xt, Y, Yt, V, Vt,
                      normYs, W, BP, constraint, aggfun, B, ...){

  ## Verify that the necessary parameters exist.
  # assertthat::assert_that(
  #   all(assertthat::has_name(update, c("Tr"))),
  ...
  fullZ   <- scalarize_values(normYs = normYs,
                              ...
                              B      = BP$fullB,
                              aggfun = aggfun)
  # Calculate scalarized performance of all iincumbent for all subproblems
  aux <- normYs
  aux$Y <- normYs$Yt
  fullZt   <- scalarize_values(normYs = aux,
                              W      = W,
                              B      = BP$fullB,
                              aggfun = aggfun)
  ...
  best.indx <-
    apply(X      = (fullZ[1:(nrow(fullZ) - 1), , drop = FALSE] - fullZt[1:(nrow(fullZt) - 1), , drop = FALSE]) /
            fullZ[1:(nrow(fullZ) - 1), , drop = FALSE],
          MARGIN = 1,
          FUN    = which.min)
  ...
  do.update <- function(i, sel.indx, XY, XYt, B){
    for (j in sel.indx[i,]) {               #each element in b_i, in fitness order
      ...
      else if (used[B[i, j]] < 1)          # tests if the current element is still available
      {
