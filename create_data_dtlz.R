...
p3 <-
  ggplot(df3, aes(Evaluations, IGD, group = Priority.Function)) +
  geom_line(aes(color = Priority.Function)) #+
# geom_point(aes(shape = name, color = name))#+
p3
ggsave(filename = pathname, device = "png")
...
write_feather(df, "forboxplot_dtlz2")
write_feather(df2, "HV_gens_dtlz2")
write_feather(df3, "IGD_gens_dtlz2")
...
for (fun in fun.names) {
  ...
  Yref <-
    as.matrix(read.table(paste0("inst/extdata/pf_data/", fun, ".3D.pf")))
...
write_feather(df, "forboxplot_dtlz3")
write_feather(df2, "HV_gens_dtlz3")
write_feather(df3, "IGD_gens_dtlz3")