...
function and the Rastrigin function in  $X = \{x_1,x_2,\dots,x_D |
x\in\mathbb{R} \}$.
...
The *moead* command requires a problem definition (discussed in the previous
section), an algorithm preset, and optionally changes to the preset, logging
parameters and a seed. The *preset_moead*  command produces a number of
different presets based on combinations found on the literature. In this
example, *preset_moead("original")* produces the original MOEAD/R composition
proposed by Zhang and Li (2007). You can get a list of available presets by
running:
...
The moead command returns an object containing the final solution set, objective
values for each solution, and other information about the optimization process.
The *MOEADr* package uses S3 to implement versions of *plot* and *summary* for
this object.

*Plot* will show the pareto front for the objectives, as in the figure below.
When the number of objectives is 3 or more, the parallel coordinates plot is
also produced (see the next example). *Summary* produces information about 
the number of non-dominated and feasible solution points, the estimated ideal and 
nadir values (when available), and the IDR (when available).
