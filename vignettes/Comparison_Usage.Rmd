...
(2016)^[L. C. T. Bezerra, M. López-Ibáñez, and T. Stützle. Automatic Component-Wise Design of Multi-Objective Evolutionary Algorithms. IEEE Transactions on Evolutionary Computation, 20(3):403–417, 2016. doi: [10.1109/TEVC.2015.2474158](https://doi.org/10.1109/TEVC.2015.2474158)], employing the *Iterated Racing* procedure by Lopez-Ibanez _et al._ (2016)^[
...
Using the [**irace**](https://mlopez-ibanez.github.io/irace/) package, we automatically assemble and fine-tune a MOEA/D configuration based on the components available in the **MOEADr** package.
...
the necessary packages and basic configuration of **irace**.
...
scenario <- list(
  seed           = 123456, # Seed for the experiment
  targetRunner   = "target.runner", # Runner function (def. below)
  debugLevel     = 1,
  maxExperiments = 20000, # Tuning budget
  testNbElites   = 7,     # test all final elite configurations
  parallel       = nc
)

# Read tunable parameters (
parameters <- readParameters(system.file(package="MOEADr", file.path("extdata", "parameters.txt"), mustWork = FALSE))
```
...
### Loading results
iraceResults <- irace::read_logfile("../inst/extdata/irace-testing.Rdata")
