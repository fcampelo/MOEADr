...
for (fun in fun.names1) {
  ...
  for (my_rep in 1:repetitions) {
    ...
    # cat("iter", iter, "\n")
    PF <- scaling_Y(moead50$Y, ref1)
    small <- dominated_hypervolume(t(PF), ref = ref.point)
    
    PF <- scaling_Y(moead500$Y, ref1)
    big <- dominated_hypervolume(t(PF), ref = ref.point)
    
    PF <- scaling_Y(moead.ps.50$Y, ref1)
    moead.ps.50 <- dominated_hypervolume(t(PF), ref = ref.point)
    
    
    
    
    results <-
      rbind(results, cbind(rbind(cbind(hv = small, name = "Small pop", fun = fun),
                                 cbind(hv = big, name = "Big. pop", fun = fun),
                                 cbind(hv = moead.ps.50, name = "MOEA/D-PS", fun = fun)
                           )))
  }
}
print(results)
