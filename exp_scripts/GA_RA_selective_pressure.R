...
fitness.cumulative <- matrix(0, nrow = npop, ncol = 5)
...
while (eval < f.eval) {
  ...
  if (select == "tournament") {
    ...
    idx.tour <- sample(1:npop, replace = T)[1:2]
  }
  else {
  ...
  fitness.cumulative[,(iter%%5)+1] <- fitness.cumulative[,(iter%%5)+1] + (fitnessess - min(fitnessess)) / (max(fitnessess) - min(fitnessess))
  
  pressure.offspring[iter - 1] <-
    Kendall(offspring.count, rowSums(fitness.cumulative))$tau[1]
...
df.distance <-
  cbind(metric = (distance.pop - min(distance.pop)) / (max(distance.pop) - min(distance.pop)), name = "Diversity all vs all")
...
print(
  ggplot(plot.data, aes(iterations, metric)) +
    ...
    ggtitle(paste("Selective pressure in GA using selection - ", select, " - in the Sphere problem."))
)
